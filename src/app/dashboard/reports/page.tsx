...
import { formatHours, formatDate } from '@/lib/utils/format';
...
export default function ReportsPage() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    const { data: ojts } = await supabase
      .from('profiles').select('*').eq('role', 'ojt').eq('is_active', true);
    ...
    const { data: allAtt } = await supabase
      .from('attendance').select('user_id, total_hours, date').not('total_hours', 'is', null);

    const { data: monthAtt } = await supabase
      .from('attendance').select('user_id, total_hours, date')
  ...
  const totalPresent = reports.filter((r) => r.this_month_days > 0).length;
